...
class PromoCode(models.Model):
    ...
    usage_limit = models.PositiveIntegerField(
        "Лимит использований", null=True, blank=True
    )
    used_count = models.PositiveIntegerField("Количество использований", default=0)
    one_time_per_user = models.BooleanField(
        "Ограничить 1 раз на клиента", default=False
    )
    used_clients = models.ManyToManyField(
        Client,
        blank=True,
        related_name="used_promo_codes",
        verbose_name="Клиенты, использовавшие промокод",
    )
    ...
    def is_active(self, client=None):
        ...
        if (
            self.one_time_per_user
            and client is not None
            and self.used_clients.filter(pk=client.pk).exists()
        ):
            return False
