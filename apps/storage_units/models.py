...
class Box(models.Model):
    ...
    floor = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(1)], verbose_name="Этаж"
    )
    length = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(1)], verbose_name="Длина"
    )
    width = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(1)], verbose_name="Ширина"
    )
...
class Tariff(models.Model):
    ...
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Цена в месяц (руб.)"
    )
    min_square_meters = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(1)], verbose_name="Минимальная площадь бокса (м²)"
    )
    ...
    description = models.TextField(blank=True, verbose_name="Подробное описание тарифа")
...
class Rental(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.qr_code:
            ...
            self.qr_code = generate_qr_code_image(
                url, filename=f"qr_{self.qr_token}.png"
            )
        super().save(*args, **kwargs)
